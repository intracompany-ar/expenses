	...
	<nav class="bg-gray-800 p-4">
		<div class="container mx-auto flex items-center justify-between">
			...
			<div class="flex space-x-4">
				<a href="#" class="text-white hover:text-gray-300">{{ auth.user?.name }}</a>
				<a href="#" class="text-white hover:text-gray-300">
					<div class="relative inline-block text-left">
						...
						<div v-show="isOpen" @click.away="isOpen = false"
							...
							<button class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" v-if="auth.isLoggedIn"
								v-on:click="auth.logout">Cerrar sesión</button>
						</div>
					</div>
				</a>
			</div>
